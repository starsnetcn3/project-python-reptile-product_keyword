...
def handle_data(lang):
    ...
    time_pattern = (
        r"(\d{4})年(\d{1,2})月(\d{1,2})日\([^)]+\)\s*(上午|下午)(\d{1,2}:\d{2})正"
    )
    ...
    for cols in colums:
        ...
        if not wuye_flag:
            ...
            # 正则表达式匹配姓名和电话
            matches = re.findall(
                r"([\u4e00-\u9fa5]+)\s*(\d{4}\s*\d{4}|\d{8})", contact_person
            )
            ...
            row_data["property_number"] = (
                property_match.group(1) if property_match else ""
            )
...
if __name__ == "__main__":
    ...
    for cid in values:
        for lang in ["tc", "sc"]:
            url = f"http://mwal.com.hk/{lang}/{cid}"
            # 暂且先写死
            # cid= cid
