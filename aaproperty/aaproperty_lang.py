...
def scrape_aa_property(url, bid):
    ...
    try:
        ...
        for row in table.find_all("tr")[7:-1]:  # 跳过表头
            ...
            print("cols", cols[2])
            row_data["image"] = (
                "https://www.aaproperty.com.hk/aa/" + cols[0].find("img")["src"]
                if cols[0].find("img")
                else ""
            )
...
def handle_data():
    ...
    for cols in colums:
        ...
        row_data = {
            ...
            "auction_id": str(inserted_id),
            "detail_url": "",
            "floor_plans": [],
        }
        print("colums", cols)
        row_data["property_address"] = clean_property_address(
            cols.get("property_address")
            .strip()
            .replace("\n", "")
            .replace("\t", "")
            .replace("\r", "")
        ...
        if not wuye_flag:
            ...
            if item_no_match:
                item_no = item_no_match.group(
                    0
                )  # 获取匹配的字符串，格式为 item_no=20250725

                # 找到所有包含 item_no 的 <a> 标签
                target_links = soup.find(
                    "a", href=lambda href: href and item_no in href
                )
                if target_links:
            ...
            try:
                ...
                if floor_plan_element:
                    floor_plan_href = (
                        "https://www.aaproperty.com.hk/aa/" + floor_plan_element["href"]
                    )
            ...
            building_area_match = re.search(
                r"建築\(約\)\s*([\d+]+(?:\+\w+\d+)*)", area_text
            )

            # 提取实用面积（包括可能的叠加情况） - 修改后的正则表达式
            saleable_area_match = re.search(
                r"實用\(約\)\s*((?:\d+\+\w+\d+|\d+)(?:\+\w+\d+)*)", area_text
            )
            print(
                "Matched saleable area:",
                saleable_area_match.group(1) if saleable_area_match else "No match",
            )
            ...
            row_data["viewing_time"] = (
                cols.get("viewing_time")
                .strip()
                .replace("\n", "")
                .replace("\t", "")
                .replace("\r", "")
            ...
            # 正则表达式匹配姓名和电话
            matches = re.findall(
                r"(\d{4}\s*\d{4}|\d{8})\s*([\u4e00-\u9fa5]+)", contact_person
            )
            ...
            row_data["completion_year"] = (
                cols.get("completion_year")
                .strip()
                .replace("\n", "")
                .replace("\t", "")
                .replace("\r", "")
            ...
            property_number_match = re.search(
                r"物業編號:\s*(\d+)", row_data["property_address"]
            )
            # 如果找到，提取数字；否则默认为 ''
            print("property_match", property_number_match)
            row_data["property_number"] = (
                property_number_match.group(1) if property_number_match else ""
            )
            ...
            with open(json_detail_filename, "w", encoding="utf-8") as json_file:
